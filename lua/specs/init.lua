...
function M.should_show_specs(start_win_id)
  ...
  ok, buftype = pcall(vim.api.nvim_win_get_option, 0, 'buftype')
  ...
  ok, filetype = pcall(vim.api.nvim_win_get_option, 0, 'filetype')
...
function M.show_specs(popup)
  ...
  vim.loop.timer_start(timer, _opts.popup.delay_ms, _opts.popup.inc_ms, vim.schedule_wrap(function()
    ...
    if vim.api.nvim_win_is_valid(win_id) then
      ...
      if bl ~= nil then
        -- vim.api.nvim_set_option_value('winblend', win_id, { winblend = bl })
